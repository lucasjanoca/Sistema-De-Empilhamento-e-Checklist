...
Data: 26/09/2026. Base da `main` remota: commit `183ee22`, preservado na tag `checkpoint-main-pre-backend-20260926`. A versão 2.1.1 introduz a migration inicial `0001_operational`; ambientes que já executavam a branch técnica permanecem sem nova alteração de schema.
...
- `python scripts/homologate.py`: lint, fronteira pública, contratos de UI/PWA, segredos, 35 testes, dependências, migrations, cadeia de auditoria e diagnóstico.
- `35 passed` em PostgreSQL 17; resultado em `TEST-RESULTS.xml`.
